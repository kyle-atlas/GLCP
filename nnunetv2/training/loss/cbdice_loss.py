...
class SoftcbDiceLoss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true, t_skeletonize_flage=False):
        ...
        y_pred_fore = y_pred[:, 1:] # (2, 2, 32, 32, 32)
        y_pred_fore = torch.max(y_pred_fore, dim=1, keepdim=True)[0] # C foreground channels -> 1 channel
        y_pred_binary = torch.cat([y_pred[:, :1], y_pred_fore], dim=1) #torch.Size([2, 2, 32, 32, 32])
        y_prob_binary = torch.softmax(y_pred_binary, 1)
        y_pred_prob = y_prob_binary[:, 1]  # torch.Size([2, 32, 32, 32]) predicted probability map of foreground
        
        with torch.no_grad():
            y_true = torch.where(y_true > 0, 1, 0).squeeze(1).float() # torch.Size([2, 32, 32, 32]) ground truth of foreground
            y_pred_hard = (y_pred_prob > 0.5).float()
            # print("y_pred_hard----", y_pred_hard.shape)
...
def get_weights(mask_input, skel_input, dim, prob_flag=True):
    if prob_flag:
        ...
        mask = mask_input
        skel = skel_input
    # print("mask----", mask.shape) #[2, 32, 32, 32]
    distances = distance_transform_edt(mask).float()
    # print("distances----", distances.shape) #[2, 32, 32, 32]
...
if __name__ == '__main__':
    ...
    softcbloss = SoftcbDiceLoss()
    cbdice = softcbloss(pred,ref)
    print('loss---',cbdice)